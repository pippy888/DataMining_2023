...
class NextLocExecutor(BertBaseExecutor):
    ...
    def train(self, train_dataloader, eval_dataloader, test_dataloader=None):
        ...
        for epoch_idx in range(self.epochs):
            ...
            if (epoch_idx % self.log_every) == 0:
                message = 'Epoch [{}/{}] ({})  train_loss: {:.4f}, train_acc: {:.2f}%, val_loss: {:.4f}, val_acc: {:.2f}%, lr: {:.6f}, {:.2f}s'. \
                    format(epoch_idx, self.epochs, (epoch_idx + 1) * num_batches, train_avg_loss,
                           train_avg_acc, eval_avg_loss, eval_avg_acc, log_lr, (end_time - start_time))
            ...
            if eval_avg_loss < min_val_loss:
                ...
                if self.saved:
                    ...
                    self._logger.info(
                        'Val loss decrease from {:.4f} to {:.4f}, saving to {}'.format(min_val_loss, eval_avg_loss,
                                                                                       model_file_name))
        ...
        if len(train_time) > 0:
            self._logger.info(
                'Trained totally {} epochs, average train time is {:.3f}s, average eval time is {:.3f}s, average train acc is {:.2f}%, average eval acc is {:.2f}%'.
                format(len(train_time), sum(train_time) / len(train_time),
                       sum(eval_time) / len(eval_time), np.mean(train_acc), np.mean(eval_acc)))
        if self.load_best_epoch:
            self.load_model_with_epoch(best_epoch)

        self._draw_png([(train_loss, eval_loss, 'loss'), (lr_list, 'lr'), (train_acc, eval_acc, 'acc')])
        return min_val_loss, np.mean(train_acc), np.mean(eval_acc)
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), desc="Train epoch={}".format(
            ...
            # print("Predictions contains NaN:", torch.isnan(predictions).any())
            # print("Predictions contains Inf:", torch.isinf(predictions).any())
            #
            # print("Criterion type:", type(self.criterion))
            # print("Criterion reduction:", self.criterion.reduction)
            # print("Criterion device:", next(self.criterion.parameters()).device if next(self.criterion.parameters(),
            #                                                                             None) is not None else "None")
            # print("Predictions shape:", predictions.shape)
            # print("Predictions sample:", predictions[0])
            # print("Targets shape:", targets.shape)
            # print("Targets sample:", targets[:10])

            # r = targets.squeeze()
            # rr = targets.squeeze(1)
            # print(r)
            # print(rr)
            r = targets.squeeze()

            batch_loss_list = self.criterion(predictions, r)
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, mode='Eval'):
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(eval_dataloader), desc="{} epoch={}".format(
                ...
                batch_loss_list = self.criterion(predictions, targets.squeeze(1))
                batch_loss = torch.sum(batch_loss_list)
                num_active = len(batch_loss_list)  # batch_size
                mean_loss = batch_loss / num_active  # mean loss (over samples) used for optimization
